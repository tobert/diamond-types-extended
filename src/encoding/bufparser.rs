...
impl<'a> BufParser<'a> {
    ...
    pub(crate) fn peek_u32(&self) -> Result<Option<u32>, ParseError> {
        ...
        Ok(Some(decode_prefix_varint_u32(self.0)?.0))
    }

    pub(crate) fn next_u32(&mut self) -> Result<u32, ParseError> {
        self.check_not_empty()?;
        let (val, count) = decode_prefix_varint_u32(self.0)?;
        self.consume(count);
        Ok(val)
    }
    ...
    pub(crate) fn next_u64(&mut self) -> Result<u64, ParseError> {
        self.check_not_empty()?;
        let (val, count) = decode_prefix_varint_u64(self.0)?;
        self.consume(count);
        Ok(val)
    }
    ...
    pub(crate) fn next_usize(&mut self) -> Result<usize, ParseError> {
        self.check_not_empty()?;
        let (val, count) = decode_prefix_varint_usize(self.0)?;
        self.consume(count);
        Ok(val)
    }
