...
impl PartialEq<Self> for CausalGraph {
    fn eq(&self, other: &Self) -> bool {
        ...
        for (mut txn, mut crdt_id) in rle_zip(
            self.iter_parents(),
            self.agent_assignment.client_with_lv.iter().map(|pair| pair.1)
        ) {
            ...
            loop {
                ...
                let (other_txn_entry, offset) = other.graph.entries.find_packed_with_offset(other_time);
                ...
                let mapped_txn = GraphEntrySimple {
