...
pub(crate) fn random_str(len: usize, rng: &mut SmallRng, use_unicode: bool) -> String {
    ...
    for _ in 0..len {
        ...
        str.push(charset[rng.random_range(0..charset.len())]);
            // str.push(UCHARS[rng.random_range(0..UCHARS.len())]);
            // str.push(alphabet[rng.random_range(0..alphabet.len())]);
    }
    str
}
...
pub(crate) fn choose_2<'a, T>(arr: &'a mut [T], rng: &mut SmallRng) -> (usize, &'a mut T, usize, &'a mut T) {
    loop {
        ...
        let a_idx = rng.random_range(0..arr.len());
        let b_idx = rng.random_range(0..arr.len());
